 ...
 * @author tamara
 */
public class UcitajPrijavuPronalaskaPsaSO extends OpstaSistemskaOperacija{
    ...
    protected ServerskiOdgovor izvrsiKonkretnuOperaciju(Object object) throws Exception {
        try{
            ...
            if(lista.isEmpty()==false){
                ...
                so.setPoruka("Sistem je učitao prijavu pronalaska psa. ");
                so.setUspesno(true);     
            }else{
                //so.setOdgovor(null); to je default
                so.setPoruka("Sistem ne može da učita prijavu pronalaska psa.");
                so.setUspesno(false);     
            }
        return so;
        }catch(SQLException ex){
            Logger.getLogger(UcitajPrijavuPronalaskaPsaSO.class.getName()).log(Level.SEVERE,null,ex);
            throw new Exception("Sistem ne može da učita prijavu pronalaska psa.");
        }
    }
