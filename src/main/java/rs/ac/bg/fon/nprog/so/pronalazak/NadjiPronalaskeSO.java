 ...
 * @author tamara
 */
public class NadjiPronalaskeSO extends OpstaSistemskaOperacija{
     @Override
    ...
    protected ServerskiOdgovor izvrsiKonkretnuOperaciju(Object object) throws Exception {
         try{
            ...
            if(lista.isEmpty()==false){
                ...
                so.setOdgovor(lista);
                so.setPoruka("Sistem je našao pronalaske po zadatoj vrednosti.");
                so.setUspesno(true);     
            }else{
                //so.setOdgovor(null); to je default
                so.setPoruka("Sistem ne može da nađe pronalaske po zadatoj vrednosti.");
                so.setUspesno(false);     
            }
        return so;
        }catch(SQLException ex){
            Logger.getLogger(NadjiPronalaskeSO.class.getName()).log(Level.SEVERE,null,ex);
            throw new Exception("Sistem ne može da nađe pronalaske po zadatoj vrednosti.");
        }
    }
