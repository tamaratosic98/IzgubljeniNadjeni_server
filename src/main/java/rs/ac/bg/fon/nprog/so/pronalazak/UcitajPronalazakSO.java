 ...
 * @author tamara
 */
public class UcitajPronalazakSO extends OpstaSistemskaOperacija{
    ...
    protected ServerskiOdgovor izvrsiKonkretnuOperaciju(Object object) throws Exception {
         try{
            ...
            List<IOpstiDomenskiObjekat> lista=DBBroker.getInstance().select((Pronalazak)object);//insert ako hocemo da se cuva
            if(lista.isEmpty()==false){
                //pakujemo parametre u serverski odgovor
                so.setOdgovor(lista.get(0));
                so.setPoruka("Sistem je učitao pronalazak.");
                so.setUspesno(true);     
            }else{
                //so.setOdgovor(null); to je default
                so.setPoruka("Sistem ne može da učita pronalazak.");
                so.setUspesno(false);     
            }
        return so;
        }catch(SQLException ex){
            Logger.getLogger(UcitajPronalazakSO.class.getName()).log(Level.SEVERE,null,ex);
            throw new Exception("Sistem ne može da učita pronalazak.");
        }
    }
